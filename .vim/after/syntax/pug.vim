...
syn region  pugJavascript start="^\z(\s*\)script\.$" end="^\%(\z1\s\|\s*$\)\@!" contains=@htmlJavascript,pugJavascriptTag,pugCoffeescriptFilter keepend 
...
syn region pugStyleTag start="^\z(\s*\)style\%(:\w\+\)\=" end="$" contains=pugBegin,pugTag
syn region pugCssBlock start="^\z(\s*\)style" end="^\%(\z1\s\|\s*$\)\@!" contains=@htmlCss,pugStyleTag keepend
syn region pugStylusBlock start="^\z(\s*\)style([^)]*type=[\"']text/stylus[\"'].*$" end="^\%(\z1\s\|\s*$\)\@!" contains=@htmlStylus,pugStyleTag keepend
